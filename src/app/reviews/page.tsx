...
export default async function ReviewsPage() {
  ...
  const renderAssignment = (a: (typeof assignments)[number]) => {
    ...
    <div>
      ...
      <ul className="mb-6 divide-y divide-gray-200 rounded border border-gray-200 bg-white">
        ...
        {activeAssignments.length === 0 && (
          <li className="px-4 py-3 text-sm text-gray-500">진행중인 심사가 없습니다.</li>
        )}
      </ul>

      <h2 className="mb-2 text-sm font-semibold text-gray-900">완료된 심사</h2>
      <ul className="divide-y divide-gray-200 rounded border border-gray-200 bg-white">
        {completedAssignments.map(renderAssignment)}
        {completedAssignments.length === 0 && (
          <li className="px-4 py-3 text-sm text-gray-500">완료된 심사가 없습니다.</li>
        )}
      </ul>
    </div>
  );
